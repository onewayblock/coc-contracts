...
describe('NFT Contract', function () {
  ...
  describe('Deployment', function () {
    ...
    it('Should revert if backend signer is zero address', async function () {
      const NFTFactory = await ethers.getContractFactory('NFT');
      await expect(
        NFTFactory.deploy('TestNFT', 'TNFT', CONTRACT_URI, ethers.ZeroAddress, [], owner.address, owner.address, 1000)
    ...
    it('Should revert if royalty receiver is zero address', async function () {
      const NFTFactory = await ethers.getContractFactory('NFT');
      await expect(
        NFTFactory.deploy('TestNFT', 'TNFT', CONTRACT_URI, backendSigner.address, [], owner.address, ethers.ZeroAddress, 1000)
      ).to.be.revertedWithCustomError(nft, 'InvalidAddress');
    });
    ...
    it('Should revert if royalty basis points are greater than 10000', async function () {
      const NFTFactory = await ethers.getContractFactory('NFT');
      await expect(
        NFTFactory.deploy('TestNFT', 'TNFT', CONTRACT_URI, backendSigner.address, [], owner.address, owner.address, 10001)
  ...
  describe('setBackendSigner', function () {
    ...
    it('Should revert if non-owner tries to update the backend signer', async function () {
      await expect(nft.connect(user).setBackendSigner(user.address)).to.be.revertedWithCustomError(nft, 'OwnableUnauthorizedAccount');
    });

    it('Should revert if new backend signer is zero address', async function () {
      await expect(nft.connect(owner).setBackendSigner(ethers.ZeroAddress)).to.be.revertedWithCustomError(nft, 'InvalidAddress');
    });
  });
  ...
  describe('addWhitelistedContract', function () {
    ...
    it('Should revert if the contract is already whitelisted', async function () {
      await expect(nft.connect(owner).addWhitelistedContract(whitelistedContract.address)).to.be.revertedWithCustomError(nft, 'AlreadyWhitelisted');
    });

    it('Should revert if non-owner tries to whitelist a contract', async function () {
      await expect(nft.connect(user).addWhitelistedContract(user.address)).to.be.revertedWithCustomError(nft, 'OwnableUnauthorizedAccount');
    });
  });
  ...
  describe('removeWhitelistedContract', function () {
    ...
    it('Should revert if the contract is not whitelisted', async function () {
      await expect(nft.connect(owner).removeWhitelistedContract(user.address)).to.be.revertedWithCustomError(nft, 'NotWhitelisted');
    });

    it('Should revert if non-owner tries to remove a contract from whitelist', async function () {
      await expect(nft.connect(user).removeWhitelistedContract(whitelistedContract.address)).to.be.revertedWithCustomError(nft, 'OwnableUnauthorizedAccount');
    });
  });
  ...
  describe('mint', function () {
    ...
    it('Should revert if mint is called by a non-whitelisted contract', async function () {
      const metadata = [JSON.stringify({ id: 1, name: 'NFT 1' })];
      await expect(nft.connect(user).mint(user.address, 1, metadata)).to.be.revertedWithCustomError(nft, 'NotWhitelisted');
    });

    it('Should revert if metadata count does not match quantity', async function () {
      const metadata = [JSON.stringify({ id: 1, name: 'NFT 1' })];
      await expect(nft.connect(whitelistedContract).mint(user.address, 2, metadata)).to.be.revertedWithCustomError(nft, 'InvalidMetadataCount');
    });
  });
  ...
  describe('mintWithSameMetadata', function () {
    ...
    it('Should revert if mintWithSameMetadata is called by a non-whitelisted contract', async function () {
      const metadata = JSON.stringify({ id: 1, name: 'NFT' });
      await expect(nft.connect(user).mintWithSameMetadata(user.address, 2, metadata)).to.be.revertedWithCustomError(nft, 'NotWhitelisted');
    });
  });
  ...
  describe('updateMetadata', function () {
    ...
    it('Should revert if signature is invalid', async function () {
      ...
      await expect(nft.connect(user).updateMetadata(tokenId, newMetadata, timestamp, invalidSignature)).to.be.revertedWithCustomError(nft, 'InvalidSigner');
    });
    ...
    it('Should revert if the same signature is used twice (replay protection)', async function () {
      ...
      await nft.connect(user).updateMetadata(tokenId, newMetadata, timestamp, signature);
      await expect(nft.connect(user).updateMetadata(tokenId, newMetadata, timestamp, signature)).to.be.revertedWithCustomError(nft, 'InvalidMessageHash');
    });
  });
  ...
  describe('updateMetadataBatch', function () {
    ...
    it('Should revert if signature is invalid', async function () {
      ...
      await expect(nft.connect(user).updateMetadataBatch(tokenIds, newMetadatas, timestamp, signatures)).to.be.revertedWithCustomError(nft, 'InvalidSigner');
    });
    ...
    it('Should revert if the same signature is used twice (replay protection)', async function () {
      ...
      await expect(nft.connect(user).updateMetadataBatch(tokenIds, newMetadatas, timestamp, signatures)).to.be.revertedWithCustomError(
        nft,
        'InvalidMessageHash'
      );
    });
    it('Should revert if duplicate token IDs are provided', async function () {
      const timestamp = new Date().getTime();
      const initialMetadata = JSON.stringify({ id: 1, name: 'NFT 1' });
      await nft.connect(whitelistedContract).mint(user.address, 1, [initialMetadata]);

      const tokenIds = [0, 0];
      const newMetadata1 = JSON.stringify({ id: 1, name: 'Updated NFT' });
      const newMetadata2 = JSON.stringify({ id: 1, name: 'Updated NFT Duplicate' });
      const newMetadatas = [newMetadata1, newMetadata2];
  ...
  describe('Royalty System', function () {
    ...
    it('Should allow owner to update royalty info', async function () {
      ...
      await expect(nft.connect(owner).setRoyaltyInfo(newReceiver, newBasisPoints)).to.emit(nft, 'RoyaltyInfoUpdated').withArgs(newReceiver, newBasisPoints);
      const [receiver, royaltyAmount] = await nft.royaltyInfo(1, SALE_PRICE);
    ...
    it('Should revert if non-owner tries to update royalty info', async function () {
      await expect(nft.connect(user).setRoyaltyInfo(user.address, 800)).to.be.revertedWithCustomError(nft, 'OwnableUnauthorizedAccount');
    });

    it('Should revert when setting royalty receiver to zero address', async function () {
      await expect(nft.setRoyaltyInfo(ethers.ZeroAddress, 500)).to.be.revertedWithCustomError(nft, 'InvalidAddress');
    });

    it('Should revert when setting royalty basis points above 10000', async function () {
      await expect(nft.setRoyaltyInfo(user.address, 10001)).to.be.revertedWithCustomError(nft, 'InvalidRoyaltyBasisPoints');
    });
  });
  ...
  describe('Transfer Lock Mechanism', function () {
    ...
    it('Should allow the owner to unlock transfers', async function () {
      await expect(nft.connect(owner).unlockTransfers()).to.emit(nft, 'TransfersUnlocked');

      expect(await nft.transferLocked()).to.be.false;
    });

    it('Should revert if non-owner tries to unlock transfers', async function () {
      await expect(nft.connect(user).unlockTransfers()).to.be.revertedWithCustomError(nft, 'OwnableUnauthorizedAccount');
    });
    ...
    it('Should prevent transfers when locked', async function () {
      ...
      await expect(nft.connect(user).transferFrom(user.address, owner.address, 0)).to.be.revertedWithCustomError(nft, 'TransfersAreLocked');
    });
